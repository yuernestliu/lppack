# lppack — Ladderpath Python Package
...
* *Ladderpath: An Efficient Algorithm for Revealing Nested Hierarchy in Sequences*, 2025.
...
To ensure experiments and results are reproducible across systems, please note the tested environment:
...
This project relies mostly on Python’s standard library.
Only **graphviz** needs to be installed manually for drawing laddergraphs.

### Python Standard Library (no manual installation required)

The following modules are included in Python and require no extra setup:
...
### External Dependency (for visualization)
...
Depending on your system, you may also need the Graphviz system package:
...
**Windows**
Download the installer from the official Graphviz website and add it to PATH.
...
A ready-to-run example is provided in `demo.ipynb`.
...
The notebook demonstrates how to use the core features of **lppack**.
...
print(lpjson["ladderpath-index"], lpjson["order-index"], lpjson["size-index"])
```

If you want to estimate eta (η), see Section 2.2 below and provide explicit parameters.
...
```
lppack/
  ladderpath/                  # core Ladderpath algorithms
  ladderpath_tools/            # auxiliary tools (compression, graph utils)
  demo.ipynb                   # runnable demo notebook
  README.md                    # this file
```
